...
export default function DocDetail({ item, tab, onClose, token, onRequestLogin }: Props) {
  ...
  useEffect(() => { setActiveTab('info'); }, [item?.id]);
    ...
    <div className="flex-1 flex flex-col bg-white overflow-hidden min-w-[280px]">
      ...
      {activeTab === 'info' ? (
        <div className="flex-1 overflow-y-auto p-4">
          <p className="text-sm text-gray-800 font-medium leading-relaxed mb-3">{doc.ten}</p>

          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 mb-4">
            {doc.ngay_ban_hanh && <span>📅 {formatDate(doc.ngay_ban_hanh)}</span>}
            {doc.loai && <span>📋 {LOAI_LABELS[doc.loai] || doc.loai}</span>}
            {(doc as Document).category_name && <span>🏷️ {(doc as Document).category_name}</span>}
            {(item as CongVan).co_quan && <span>🏛️ {(item as CongVan).co_quan}</span>}
          </div>
          ...
          {tab === 'congvan' && (detail as CongVan)?.ket_luan && (
            <div className="mt-4">
              ...
              <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-wrap">{(detail as CongVan).ket_luan}</p>
            </div>
          )}
          ...
          <div className="mt-6 flex gap-2">
            {(doc as Document).github_path && (
              <a href={`https://vntaxdoc.gpt4vn.com/docs/${(doc as Document).github_path}`}
            ...
            {(item as CongVan).link_nguon && (
              <a href={(item as CongVan).link_nguon}
                target="_blank" rel="noopener noreferrer"
